    ...
    for i, sat in enumerate(list_satellites):
        ...
        if excluded_satellites is not None:
            for j, PRN in enumerate(excluded_satellites):
                print(f"DEBUG:   PRN[{j} of {np.size(excluded_satellites)}] = {PRN}")
                if PRN in sat.name:
                    line_style = "--"
...
def plot_dop_visible(
    ...
    for i in range(0, 3):
        ...
        if i == 1:
            ...
            ax1.plot(
                prediction_dt, np.sqrt(PDOP2), linewidth=2, color=dopColor, label="PDOP"
            )
        ...
        if i == 2:
            ...
            dopColor = next(colors)
            transparency = 0.1
            ax1.fill_between(prediction_dt, 0, GDOP, color=dopColor, alpha=transparency)
