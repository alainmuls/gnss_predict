...
def parse_arguments(script_name: str) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=f"{script_name} uses NORAD TLE files to predict GNSS orbits"
    )
    ...
    parser.add_argument(
        "-i", "--interval", help="Interval in minutes", type=int, default=20
    )

    parser.add_argument(
        "-c", "--cutoff", help="Cutoff angle in degrees", type=int, default=10
    )
    ...
    parser.add_argument("-s", "--start", help="Start time (hh:mm)", default="00:00")

    parser.add_argument("-e", "--end", help="End time (hh:mm)", default="23:59")

    parser.add_argument(
        "-m", "--max-dop", help="Maximum xDOP value to display", type=int, default=10
    )
